  ...
  'inline-flex items-center cursor-pointer justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0',
  {
    variants: {
      variant: {
        ...
        cool: 'dark:inset-shadow-2xs dark:inset-shadow-white/10 bg-linear-to-t border border-b-2 border-zinc-950/40 from-primary to-primary/85 shadow-md shadow-primary/20 ring-1 ring-inset ring-white/25 transition-[filter] duration-200 hover:brightness-110 active:brightness-90 dark:border-x-0 dark:border-t-0 dark:border-primary/50 dark:ring-white/5 text-white',
...
const liquidbuttonVariants = cva(
  'inline-flex items-center justify-center cursor-pointer gap-2 whitespace-nowrap rounded-full text-sm font-semibold transition-all duration-300 disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg:not([class*=size-])]:size-4 shrink-0 [&_svg]:shrink-0 outline-none focus-visible:ring-[3px] focus-visible:ring-ring/50',
  {
    variants: {
      variant: {
        default:
          'text-ink bg-white/4 border border-white/46 shadow-[0_10px_30px_rgba(140,180,210,0.08)] backdrop-blur-2xl hover:bg-white/10 hover:border-white/62 hover:-translate-y-0.5 [text-shadow:0_1px_0_rgba(255,255,255,0.55)]',
        destructive:
          'text-white bg-red-500/10 border border-red-200/44 backdrop-blur-2xl hover:bg-red-500/18',
        outline:
          'text-ink bg-white/3 border border-white/42 backdrop-blur-2xl hover:bg-white/9 hover:border-white/56 [text-shadow:0_1px_0_rgba(255,255,255,0.5)]',
        secondary:
          'text-ink-mid bg-white/4 border border-white/40 backdrop-blur-2xl hover:bg-white/10',
        ghost: 'text-ink bg-transparent hover:bg-white/9',
        link: 'text-primary underline-offset-4 hover:underline',
      },
      size: {
...
export function LiquidButton({
  ...
  return (
    <Comp data-slot="button" className={cn('relative', liquidbuttonVariants({ variant, size, className }))} {...props}>
      <div className="pointer-events-none absolute inset-0 z-0 rounded-full border border-white/58 bg-gradient-to-b from-white/18 via-white/6 to-white/2" />
      <div
        className="pointer-events-none absolute inset-0 isolate z-0 overflow-hidden rounded-full"
        style={{ backdropFilter: 'url("#container-glass") blur(24px) saturate(185%)' }}
      />
      <div className="pointer-events-none absolute inset-x-3 top-[1px] z-0 h-[40%] rounded-full bg-gradient-to-b from-white/74 to-transparent" />
      <div className="relative z-10 inline-flex items-center gap-2 whitespace-nowrap">{children}</div>
      <GlassFilter />
    </Comp>
  );
}
